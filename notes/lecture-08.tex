...
\be
 ...
 \overline{D(0,r)},
\ee
which is an instance of the comparison test.

\section{The basic tool: the comparison test}
...
Let $\{t_n\}$ be a sequence of nonnegative real numbers. An \emph{accumulation
point} of $\{t_n\}$ is a point $t\in\R$ such that every interval around $t$
contains $t_n$ for infinitely many indices $n$.
\end{definition}

\begin{fact}
Every bounded sequence in $\R$ has a convergent subsequence and hence an
accumulation point.
\end{fact}

\begin{definition}[$\limsup$]
Suppose $\{t_n\}$ is bounded, and let $S$ be the set of all its accumulation
points. Then
\be
...
If $\{t_n\}$ is unbounded, we set $\limsup_n t_n=\infty$.
\end{definition}

\begin{exercise}
Let $\{t_n\},\{s_n\}$ be bounded sequences of nonnegative real numbers and set
$t=\limsup_n t_n$, $s=\limsup_n s_n$. Verify:
\begin{itemize}
...
\label{thm:hadamard}
Let $r$ be the radius of convergence of $\sum a_nz^n$. Then
\be
 \frac1r=\limsup_{n}|a_n|^{1/n}.
\ee
...
radius of convergence as $r=1/t$.
